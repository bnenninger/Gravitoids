...
extern char text_buffer[8];
extern volatile uint8_t ack_received, ack_sent;
...
void I2C0Init( void ) 
{
  ...
  // cris: I2C_PCLK = CCLK/1 (page 57 of user manual)
  ...
  LPC_PINCON->PINSEL1 |= ((0x01<<22)|(0x01<<24));	
	LPC_PINCON->PINMODE3 &= ~((0x03<<22)|(0x03<<24));
  LPC_PINCON->PINMODE3 |= ((0x02<<22)|(0x2<<24));	// No pull-up no pull-down; see page 121 of datasheet
  ...
  // cris: enable clk stretching
  //http://www.avrfreaks.net/index.php?name=PNphpBB2&file=viewtopic&t=119340
  //LPC_I2C0->MMCTRL |= (1<<1);
...
void I2C1Init( void ) 
{
  LPC_SC->PCONP |= (1 << 19); // sets power for I2C1; see page 63 of manual;

#if 0
  /* set PIO0.0 and PIO0.1 to I2C1 SDA and SCL */
  /* function to 11 on both SDA and SCL. */
  LPC_PINCON->I2PINSEL0 &= ~((0x3<<0)|(0x3<<2));
  LPC_PINCON->I2PINSEL0 |= ((0x3<<0)|(0x3<<2));
  LPC_PINCON->I2PINMODE0 &= ~((0x3<<0)|(0x3<<2));
  LPC_PINCON->I2PINMODE0 |= ((0x2<<0)|(0x2<<2));		/* No pull-up no pull-down */
  LPC_PINCON->I2PINMODE_OD0 |= ((0x01<<0)|(0x1<<1));	/* Open drain */	
#endif
#if 1
    
  /* set PIO0.19 and PIO0.20 to I2C1 SDA and SCL */
  /* function to 11 on both SDA and SCL. */
  LPC_PINCON->PINSEL1 &= ~((0x3<<6)|(0x3<<8)); // pin connect block connects: SDA1 to P0.19
  LPC_PINCON->PINSEL1 |= ((0x3<<6)|(0x3<<8));  // and also: SCL1 to P0.20
  LPC_PINCON->PINMODE1 &= ~((0x3<<6)|(0x3<<8));
  LPC_PINCON->PINMODE1 |= ((0x2<<6)|(0x2<<8));	/* No pull-up no pull-down */
  // cris: use resistors 
  //LPC_PINCON->I2PINMODE1 &= ~((0x3<<6)|(0x3<<8));
  //LPC_PINCON->I2PINMODE1 |= ((0x3<<6)|(0x3<<8));
...
{
    WrIndex0 = 0; WrIndex1 = 0; WrIndex2 = 0;
    RdIndex0 = 0; RdIndex1 = 0; RdIndex2 = 0;
    ack_received = 0; ack_sent = 0;
