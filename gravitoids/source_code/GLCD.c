//-----------------------------------------------------------------------------
// Version 1 (nu include de exemplu cum sa desenezi cerc umplut)
//
// Cris:
...
static __attribute__((always_inline)) uint16_t LCD_Read(void)
{
  ...
  wait_delay(TIME_TO_DELAY);                 // delay some times
...
//                                                                                                  Description : Bresenham's line algorithm Input : -x1 : -y1 : -x2 : -y2 : -color :
//
//                                                                                                                this function seems to be horribly broken,
// use LCD_draw_line instead
...
//     of open source code;
// Note to myself : in previous editions of this course, I had these in separate files CRIS_UTILS.h and CRIS_UTILS.c;
// now, they are here to
//          work with fewer files;
// this is simply the midpoint circle algorithm for drawing a circle;
// it is a variant of Bresenham's line algorithm, and is thus sometimes known as Bresenham's circle algorithm; see additional info : http :               //en.wikipedia.org/wiki/Midpoint_circle_algorithm
...
void LCD_draw_circle(int x0, int y0, int radius, uint16_t color)
{
  ...
  while (x >= y)
  {
    LCD_SetPoint(x + x0, y + y0, color); // call of GCLD function;
    LCD_SetPoint(y + x0, x + y0, color);
...
// Note: GLCD.c that comes with the LandTiger 2.0 board has now an
// implementation of the same algorithm: LCD_DrawLine() (see above)
// here, I have LCD_draw_line() since the time when it was not available
// with the examples of MCB1700 boards... so, it's here for historical reasons;
