...
uint16_t* screen = NULL;			//The current screen that needs to be updated
...
int main (int argc, char* args[]) {
	...
	int sleep = 0;
	int quit = 0;
	//TODO: change event to some other input method
	SDL_Event event;
	...
	while(quit == 0) {
		...
		clear_pixels(pixels, Black);
		//updated clear, will clear only objects that are currently drawn
		//added in , clear pixels from screen as well
		clear_pixels(screen, Black);
		...
		//SDL_UpdateTexture(screen, NULL, pixels, MAX_X * sizeof (Uint32));

		//draw to the screen
		//SDL_RenderClear(renderer);
		//SDL_RenderCopy(renderer, screen, NULL, NULL);
		//SDL_RenderPresent(renderer);
		//Instead, GLCD drawing code directly to screen, pixel-by-pixel
...
int init(int width, int height) {
	...
	//Create window	
	SDL_CreateWindowAndRenderer(MAX_X, MAX_Y, SDL_WINDOW_SHOWN, &window, &renderer);
	
	//set up screen texture
	screen = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, MAX_X, MAX_Y);
	
	//allocate pixel buffer
	pixels = (uint16_t*) malloc((MAX_X * MAX_Y) * sizeof(uint16_t));
